...
กลุ่มไหนพิมพ์ **`โหมดชาเลนจ์`** จะเปลี่ยนจากนับแคลเป็นเช็คว่าใครออกกำลังกายวันนี้แล้วบ้าง
...
| (อัตโนมัติ 20:00) | ประกาศรายชื่อคนที่ยังไม่ออกกำลังกายวันนี้ |
...
> ⚠️ ข้อความ push (เตือน 20:00) นับโควตาข้อความของ LINE OA ส่วนการ reply เมื่อมีคนพิมพ์หานั้นฟรีไม่จำกัด
> ถ้ากลุ่มใหญ่แล้วโควตาไม่พอ ให้ปิด cron 20:00 แล้วใช้คำสั่ง `วันนี้` แทนได้
...
- Gemini free tier: **ฟรี** แต่มีโควตาจำกัด**ต่อรุ่นต่อวัน** (เช่น `gemini-2.5-flash` = 20 ครั้ง/วัน)
  บอทตั้งรุ่นหลักเป็น `gemini-3.5-flash-lite` (โควตาสูงกว่า) และสลับไปรุ่นสำรองอัตโนมัติเมื่อรุ่นหลักเต็ม
  ถ้าใช้หนักจนไม่พอ เปิด billing ที่ Google AI Studio ได้ (จ่ายตามใช้จริง หลักบาทต่อเดือนสำหรับ 2 คน)
